---
id: d2l-63f26ba3a8f3d0f2
title: "Customer Story - Los Angeles Pacific University - Equipping Students with Competency-Based Education: A Path to Empowerment"
type: customer-proof
status: verified
aliases: []
tags:
  - adultcontinuing-education
  - brightspace-core
  - collegeuniversity
  - credentialing
  - customer-story
  - education
  - faith-based-collegeuniversity
  - masterycompetency-based-education-cbe
  - optimization-services
  - performance-plus
  - published
  - technical-skills-university
source_system: published-customer-story
source_title: "Equipping Students with Competency-Based Education: A Path to Empowerment"
source_url: https://www.d2l.com/why-d2l/customers/equipping-students-with-competency-based-education/
source_date: 2023-05-10
captured_at: 2026-07-30T15:33:12-04:00
last_verified: 2026-07-30
review_after: 2027-01-26
source_owner: D2L Customer Marketing
confidence: high
verification_scope: record-confirmed
verification_basis: The published customer-story record, metadata, and source link were confirmed.
external_use: source-check-required
confidentiality: public
seller_relevance: Published customer proof candidate for discovery, positioning, proposals, and objection handling; validate the live page before external reuse.
related: []
related_notes:
  - title: "Brightspace Core Positioning"
    relationship: "uses-product"
    why: "This customer used Brightspace Core, D2L's foundational LMS platform, as tagged in the source record."
  - title: "Performance Plus Public Capabilities"
    relationship: "uses-product"
    why: "This customer used Performance Plus analytics capabilities, as tagged in the source record."
  - title: "Higher Education Playbook"
    relationship: "validates-context"
    why: "This customer is a higher-education institution; the playbook explains the sector context and typical buying dynamics."
---
# Customer Story - Los Angeles Pacific University - Equipping Students with Competency-Based Education: A Path to Empowerment

## Verified fact

A published customer-story record exists for Los Angeles Pacific University. This imported note has been promoted to verified vault status by human override.

## Signal

- Published story summary: How Los Angeles Pacific University (LAPU) created mobile-first, competency-based educational offerings to support learners advancing on their own terms.
- Reported impact: Empowers adult learners to engage in content on their own time and balance education with their busy lives
- Reported impact: Provides students with micro-certifications to demonstrate tangible accomplishments to potential employers
- Reported impact: Accessible, mobile-first layout accommodates diverse preferences for all students
- Products/services named in the export: Brightspace Core, Performance +, Optimization Services.
- Use cases tagged in the export: Mastery/Competency-Based Education (CBE), Micro-credentials/Micro-learning, Adult/Continuing Education.
- Learners listed: 3,000+.
- Context: North America; Education.

## Interpretation

**Interpretation:** The tagged outcomes may make this story relevant to a buyer with matching goals, but the relationship and causality must remain scoped to this customer.

## Why it matters

Provides a discoverable proof candidate without turning an index row into a broader product or outcome claim.

## Seller decision

Use as a proof candidate; recheck the live source before quoting exact wording, metric context, or permissioned external language.

## Next action

Human override completed; recheck the linked story before quoting exact wording or externally reusing specific metrics.

## Desired buyer outcome

Help the buyer evaluate a relevant, credible example while avoiding unsupported generalization.

## Objections and cautions

- Internal indexing is not proof that every exported field appears on the public page.
- Do not generalize metrics, causality, timing, or ROI beyond the named customer.
- Recheck the live source before quoting or using metrics externally.

## Sources

- [Los Angeles Pacific University customer story](https://www.d2l.com/why-d2l/customers/equipping-students-with-competency-based-education/)
- Imported from `_ All Customer Stories - Published.csv` supplied by the user.

## Change history

- 2026-07-30: Human override by Pat; marked verified with high confidence.

- 2026-07-30: Imported to `90-Inbox` from the supplied published-customer-stories CSV; verified by human override.
